# -*- coding: utf-8 -*-
"""
Created on Sat Mar  2 10:59:46 2019
...
# =============================================================================
#  Solves the HH problem under VFI and Simulates the Economy in the Primary scenario.
# =============================================================================
...
folder='C:/Users/rodri/OneDrive/Documentos/IDEA/Phd tesi/figures/model inputs/'
save= True
...
V_new = np.empty(( N_m, N_m, N_θ, N_ε))
policy_a = np.empty(( N_m, N_m, N_θ, N_ε))
policy_m1 = np.empty(( N_m, N_m, N_θ, N_ε))
policy_m2 = np.empty(( N_m, N_m, N_θ, N_ε))
policy_c = np.empty(( N_m, N_m, N_θ, N_ε))


## Generate guess on V:
V_0_func = lambda a, m1, m2, θ, ε: u(θ*ε*A*m1**α +ε*B*m2**γ -p*(m1 +m2))/(1-β)
...
#improve v_guess
V_guess = np.zeros(( N_m, N_m, N_θ, N_ε))
'''
...
m=np.empty(( N_m, N_m, N_θ, N_ε, N_m, N_m))
            ...
            for i1,i_θ in enumerate(θ_grid):
                for i2,i_ε in enumerate(ε_grid):                                                  
                    for i_gm2, g_m2 in enumerate(m_grid):                           
                        m[i_m1,i_m2,i1, i2, :,i_gm2] = u(i_θ*i_ε*A*m1**α +i_ε*B*m2**γ -p*m_grid -p*g_m2)+β*(pi_θ[i1,0]*pi_ε[i2,0]*V[:, i_gm2,0,0] +pi_θ[i1,1]*pi_ε[i2,0]*V[:, i_gm2,1,0] +pi_θ[i1,0]*pi_ε[i2,1]*V[:, i_gm2,0,1] +pi_θ[i1,1]*pi_ε[i2,1]*V[:, i_gm2,1,1])   
                            #print(m[i_m1,i_m2,i1, i2, :,i_gm1,i_gm2])
                            
                    V_new[i_m1,i_m2,i1,i2] = np.nanmax(m[i_m1,i_m2, i1,i2,:,:])
                        #print(V_new)
        ...
        for i_m2, m2 in enumerate(m_grid):
            for i1,i_θ in enumerate(θ_grid):
                for i2,i_ε in enumerate(ε_grid):                              
                    ...
                    policy_m1[i_m1,i_m2,i1,i2] = m_grid[np.unravel_index(np.argmax(m[i_m1,i_m2, i1,i2,:,:], axis=None), m[i_m1,i_m2, i1,i2,:,:].shape)[0]]
                    policy_m2[i_m1,i_m2,i1,i2] = m_grid[np.unravel_index(np.argmax(m[i_m1,i_m2, i1,i2,:,:], axis=None), m[i_m1,i_m2, i1,i2,:,:].shape)[1]]
                    policy_c[i_m1,i_m2,i1,i2] = i_θ*i_ε*A*m1**α +i_ε*B*m2**γ -p*policy_m1[i_m1,i_m2,i1,i2] -p*policy_m2[i_m1,i_m2,i1,i2] 
...
def plot_policy(policy, policy_name, m0=0, save=False, folder=folder):      
    fig,ax = plt.subplots()
    ax.plot(m_grid, np.array(policy[m0,:,1,1]), label=policy_name+r'($\theta=1, \varepsilon=1)$')
    ax.plot(m_grid, np.array(policy[m0,:,0,1]), label=policy_name+r'($\theta=0, \varepsilon=1)$')
    ax.plot(m_grid, np.array(policy[m0,:,1,0]), label=policy_name+r'($\theta=1, \varepsilon=0)$')
    ax.plot(m_grid, np.array(policy[m0,:,0,0]), label=policy_name+r'($\theta=0, \varepsilon=0)$')
    ax.legend()
...
def invariant_distr(pi_θ, pi_ε, T=1000, N=1000, θ_h_0= 0.5, ε_h_0 = 0.5):
    ...
    for t in range(1, T):        
        for n in range(0,N):
            ...
            m1_state[t,n] = g_m1[input1_list.index(m1_state[t-1,n]),input2_list.index(m2_state[t-1,n]),int(θ_shock[t-1, n]), int(ε_shock[t-1, n])]
            m2_state[t,n] = g_m2[input1_list.index(m1_state[t-1,n]),input2_list.index(m2_state[t-1,n]),int(θ_shock[t-1, n]), int(ε_shock[t-1, n])]
            c_state[t,n] =  g_c[input1_list.index(m1_state[t-1,n]),input2_list.index(m2_state[t-1,n]),int(θ_shock[t-1, n]), int(ε_shock[t-1, n])]
    ...
    return θ_shock[T-1,:], ε_shock[T-1,:], m1_state[T-1,:], m2_state[T-1,:] , y_state[T-1,:], c_state[T-1,:], mean_m1, mean_m2, mean_y, mean_c

θ_shock, ε_shock, m1_state, m2_state, y_state,c_state, mean_m1, mean_m2, mean_y, mean_c = invariant_distr(pi_θ, pi_ε, T=T, N=N, θ_h_0=θ_h_0 , ε_h_0 = ε_h_0)
